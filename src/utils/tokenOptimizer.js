...
export const PROVIDER_GENERATION_DEFAULTS = {
  openrouter: { maxTokens: 4096, temperature: 0.7, topP: 0.9 },
  openai: { maxTokens: 4096, temperature: 0.7, topP: 0.9 },
  anthropic: { maxTokens: 4096, temperature: 0.7, topP: 0.9 },
  huggingface: { maxTokens: 4096, temperature: 0.65, topP: 0.9 },
  ollama: { maxTokens: 4096, temperature: 0.7, topP: 0.9 },
};
...
export function assembleContextWindow(
  messages = [],
  { contextMode = "full", windowSize = 10, protectFiles = true } = {}
) {
  const all = Array.isArray(messages) ? messages : [];
  const size = Math.max(2, Number(windowSize) || 10);

  if (contextMode === "full" || all.length <= size) {
    return { messages: [...all], summaryNote: "" };
  }

  const recent = all.slice(-size);
  const overflow = all.slice(0, all.length - size);
  // Older turns that carry an upload are kept verbatim so files never fall out
  // of context just because the conversation got long.
  const retainedUploads = protectFiles ? overflow.filter((m) => messageCarriesUpload(m)) : [];

  if (contextMode === "smart") {
    const toSummarize = protectFiles ? overflow.filter((m) => !messageCarriesUpload(m)) : overflow;
    const summaryNote = buildFallbackHistorySummary(toSummarize, "", 1400);
    return { messages: [...retainedUploads, ...recent], summaryNote };
  }
...
// A message "carries a file/image" if it holds an inlined upload. `send.js`
// inlines text files with this exact sentinel, and vision uploads arrive as
// `image_url` parts. These are the pieces users expect the model to still see
// later in the conversation, so budget compression must leave them intact.
const INLINED_FILE_RE = /\[Attached file:/;

export function messageCarriesUpload(message) {
  const content = message?.content;
  if (Array.isArray(content)) {
    if (content.some((p) => p?.type === "image_url")) return true;
  }
  return INLINED_FILE_RE.test(extractMessageText(content));
}

/**
 * Fit `messages` into `maxInputTokens`, compressing oldest-first while keeping
 * structure. When `protectFiles` is set, messages that carry an inlined file or
 * image are never compressed — so an upload stays readable for the whole chat
 * instead of being crushed to a stub a few turns later.
 */
export function enforceInputTokenBudget(messages = [], maxInputTokens = 3000, { protectFiles = false } = {}) {
  const isProtected = (msg) => protectFiles && messageCarriesUpload(msg);
  ...
  if (estimatedTokens > budget) {
    ...
    for (let i = 1; i < next.length - 1; i++) {
      if (isProtected(next[i])) continue;
    ...
    if (estimatedTokens > budget && next.length > 0 && !isProtected(next[next.length - 1])) {
...
export function resolveGenerationSettings(provider, responseLength = "medium") {
  ...
  return {
    maxTokens: Math.max(64, Math.min(16384, Number(preset) || defaults.maxTokens)),
