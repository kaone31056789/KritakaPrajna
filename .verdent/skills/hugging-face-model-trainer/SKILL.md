...
UV scripts use PEP 723 inline dependencies for clean, self-contained training. **This is the recommended approach.**
...
**When to use:** Default choice for all training tasks when MCP tools are available, custom training logic, any scenario requiring `hf_jobs()`
...
**When to use:** User working in terminal directly (without MCP tools), quick local experimentation
**Repository:** https://github.com/huggingface/trl-jobs

⚠️ **When MCP tools are available, prefer using `hf_jobs()` (Approach 1).**
...
7. **Use UV scripts (Approach 1)** - Default to `hf_jobs("uv", {...})` with inline scripts; TRL maintained scripts for standard training; avoid bash `trl-jobs` commands when MCP tools are available
